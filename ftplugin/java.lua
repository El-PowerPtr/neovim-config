...
local config = {
    ...
    root_dir = vim.fs.dirname(vim.fs.find({ '.project', 'gradlew, mvnw, gradle.build, pom.xml', '.git' },
        { upward = true })[1]),
    init_options = {
    ...
    on_attach = function(client, bufnr)
        ...
        local opts2 = { silent = true, buffer = bufnr }

        vim.keymap.set('n', "<leader>O", jdtls.organize_imports, { desc = 'Organize imports', buffer = bufnr })
        -- Should 'd' be reserved for debug?
        vim.keymap.set('n', "<leader>tc", jdtls.test_class, opts2)
        vim.keymap.set('n', "<leader>tn", jdtls.test_nearest_method, opts2)
